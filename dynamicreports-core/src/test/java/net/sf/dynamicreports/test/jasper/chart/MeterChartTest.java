...
public class MeterChartTest extends AbstractJasperChartTest {
  ...
  private void intervalTest(final MeterInterval interval, final String label, final Color backgroundColor,
      final double rangeLow, final double rangeHigh) {
    Assertions.assertEquals(label, interval.getLabel(), "interval label");
    Assertions.assertEquals(backgroundColor,
        interval.getBackgroundPaint(), "interval background color");
    Assertions.assertEquals(rangeLow, interval.getRange().getLowerBound(),
        0, "interval data range low");
    Assertions.assertEquals(rangeHigh, interval.getRange().getUpperBound(),
        0, "interval data range high");
  }
